...
from .misc import reorder_image, to_y_channel, tensor2img
from .matlab_functions import imresize

def get_arguments():
    parser = ArgumentParser(formatter_class=ArgumentDefaultsHelpFormatter)
    parser.add_argument('--path', type=str, required=True, help=('Path to the image'))
    parser.add_argument('--pixels-to-crop', type=int, default=0, help=('pixels to crop (default: 0)'))
...
def calculate_niqe_given_tensor(tensor, crop_border=0):
    img = tensor2img(tensor)
    niqe_value = calculate_niqe(img, crop_border)
...
def calculate_niqe(img, crop_border, input_order='HWC', convert_to='y', **kwargs):
    ...
    # we use the official params estimated from the pristine dataset.
    niqe_pris_params = np.load(os.path.join(ROOT_DIR, '..', 'weights', 'niqe_pris_params.npz'))
...
if __name__ == '__main__':
    ...
    niqe_value = calculate_niqe(img, args.pixels_to_crop)

    print('NIQE: ', niqe_value)
